...
export class ToolRegistry {
  ...
  static resolvePath(workspacePath: string, targetPath: string, allowOutside = false): string {
...
ToolRegistry.register({
  name: "read_file",
  description: "Reads the content of a file in the workspace, with optional line range selection.",
  parameters: {
    path: { type: "string", description: "The path of the file to read", required: true },
...
ToolRegistry.register({
  ...
  parameters: {
    path: { type: "string", description: "The path of the file to write", required: true },
...
ToolRegistry.register({
  name: "search_files",
  description: "Performs a grep-like recursive text search for query terms across files in the workspace.",
  parameters: {
    query: { type: "string", description: "The query string to look for", required: true },
  },
  handler: async (workspace, params) => {
...
ToolRegistry.register({
  name: "run_terminal",
  description: "Executes a shell command in the workspace directory under strict timeout and safety configurations.",
  parameters: {
  ...
  handler: async (workspace, params) => {
    ...
    if (blocklist.some((term) => command.includes(term))) {
      throw new Error(`Security Exception: Command blocked. Dangerous operations detected.`);
    }

    // Safety check 2: Allowlist prefix constraints
    const allowlist = [
      "git", "npm", "node", "python", "pytest", "pip", "composer", "go", "cargo"
    ];
    const baseCommand = command.trim().split(/\s+/)[0];
    if (!allowlist.includes(baseCommand)) {
      throw new Error(`Security Exception: Command "${baseCommand}" is not in the safe allowlist.`);
    }

    // Spawn execution with 30s timeout
    return new Promise((resolve, reject) => {
      exec(command, { cwd: workspace, timeout: 30000 }, (error, stdout, stderr) => {
        if (error) {
          if (error.killed) {
            reject(new Error(`Command timed out (exceeded 30-second limit).`));
          } else {
            resolve(`Error (Exit Code ${error.code}):\n${stderr || error.message}`);
          }
          return;
        }
...
ToolRegistry.register({
  ...
  handler: async (workspace) => {
    return new Promise((resolve, reject) => {
      exec("git status", { cwd: workspace, timeout: 10000 }, (error, stdout, stderr) => {
...
ToolRegistry.register({
  name: "git_diff",
  description: "Returns the git diff for a specific file or the entire repository.",
  parameters: {
    path: { type: "string", description: "Optional path of a file to diff" },
  },
  handler: async (workspace, params) => {
    const subPath = params.path ? ToolRegistry.resolvePath(workspace, params.path) : "";
    const command = subPath ? `git diff "${subPath}"` : "git diff";

    return new Promise((resolve, reject) => {
      exec(command, { cwd: workspace, timeout: 15000 }, (error, stdout, stderr) => {
        if (error) resolve(`Git Error:\n${stderr || error.message}`);
...
ToolRegistry.register({
  ...
  handler: async (workspace) => {
    ...
    if (!fs.existsSync(pkgPath) && fs.existsSync(path.join(workspace, "requirements.txt"))) {
      testCommand = "python -m unittest";
    }

    return new Promise((resolve) => {
      exec(testCommand, { cwd: workspace, timeout: 30000 }, (error, stdout, stderr) => {
        if (error) {
...
ToolRegistry.register({
  name: "list_directory",
  description: "Lists all files and subdirectories inside a workspace directory.",
  parameters: {
    path: { type: "string", description: "Optional relative subdirectory path" },
  },
  handler: async (workspace, params) => {
...
ToolRegistry.register({
  ...
  parameters: {
    manager: { type: "string", description: "npm | pip | composer", required: true },
  ...
  handler: async (workspace, params) => {
    ...
    return new Promise((resolve, reject) => {
      exec(command, { cwd: workspace, timeout: 60000 }, (error, stdout, stderr) => {
        if (error) {
          reject(new Error(`Failed to install package: ${stderr || error.message}`));
        } else {
          resolve(`Package ${name} installed successfully using ${manager}.\n${stdout}`);
        }
      });
    });
  },
