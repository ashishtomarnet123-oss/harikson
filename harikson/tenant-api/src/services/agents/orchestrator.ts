...
export interface TaskStep {
  step: number;
  agent: "Harikson Planner" | "Harikson Research" | "Harikson Coder" | "Harikson Reviewer" | "Harikson Tester";
  task: string;
...
export class HariksonOrchestrator {
  ...
  static async executeTask(
    ...
    Logger.info("🤖 [Harikson Orchestrator] Starting multi-agent task execution...", { userRequest });
    ...
    while (currentStepIdx < planSteps.length && attempts < 15) {
      ...
      Logger.info(`⚡ Running Step ${currentStep.step}: ${currentStep.agent} - "${currentStep.task}"`);
      ...
      try {
        ...
        switch (currentStep.agent) {
          case "Harikson Research":
            result = await this.runResearchAgent(tenantId, currentStep.task, sharedContext);
            break;
          case "Harikson Coder":
            result = await this.runCoderAgent(tenantId, conversationId, currentStep.task, sharedContext);
            break;
          case "Harikson Reviewer":
            result = await this.runReviewerAgent(currentStep.task, sharedContext);
            
            // Loopback mechanism: If Reviewer rejects the code, reset the Coder step!
            if (result.toLowerCase().includes("reject")) {
              Logger.warn("⚠️ Reviewer rejected code! Resetting previous Coder step for revision.");
          ...
          case "Harikson Tester":
            result = await this.runTesterAgent(tenantId, conversationId, currentStep.task, sharedContext);
            break;
          default:
        ...
        Logger.error(`❌ Agent ${currentStep.agent} failed at Step ${currentStep.step}`, err);
    ...
    const finalOutput = sharedContext[`step_${planSteps.length}_result`] || "Multi-agent task run completed.";
    const duration = (Date.now() - start) / 1000;
    
    metrics.recordLatency("agent_orchestration", duration);
    Logger.info("🎉 [Harikson Orchestrator] Multi-agent execution finished successfully!");

    return { planId, finalOutput, steps: planSteps };
  }

  private static async generatePlan(userRequest: string): Promise<TaskStep[]> {
    const systemPrompt = "You are the Harikson Orchestration Planner. Split the user request into 4-5 sequential sub-tasks. Choose only from agents: 'Harikson Research', 'Harikson Coder', 'Harikson Reviewer', 'Harikson Tester'. Output valid JSON array matches strictly this format: [{\"step\": 1, \"agent\": \"Harikson Research\", \"task\": \"...\"}]";
    
    try {
      const response = await OllamaClient.generate(`Create a plan for: "${userRequest}"`, systemPrompt);
      const parsed = JSON.parse(response.substring(response.indexOf("["), response.lastIndexOf("]") + 1));
      ...
      return [
        { step: 1, agent: "Harikson Research", task: "Scan workspace and search for existing files or auth middleware.", status: "pending" },
        { step: 2, agent: "Harikson Coder", task: "Write code matching the user request and save file changes.", status: "pending" },
        { step: 3, agent: "Harikson Reviewer", task: "Review the code changes for syntax errors and architectural alignment.", status: "pending" },
        { step: 4, agent: "Harikson Tester", task: "Execute workspace tests to verify code stability.", status: "pending" },
      ];
    }
  }

  private static async runResearchAgent(tenantId: string, task: string, context: Record<string, string>): Promise<string> {
    const { results } = await VectorSearchService.search(task, tenantId, { code_only: true }, 5, 0);
    if (results.length === 0) return "No relevant code chunks discovered in repository search.";
    return `Discovered matching code:\n` + results.map((r) => `- File ${r.source_path}\n${r.content}`).join("\n");
  }
  ...
  private static async runCoderAgent(
    ...
    const systemPrompt = "You are Harikson Coder. Generate the write_file tool call XML tags required to implement the task.";
    const response = await OllamaClient.generate(`Implement this: "${task}"`, systemPrompt);

    const parsedCalls = ToolExecutor.parseToolCalls(response);
    if (parsedCalls.length > 0) {
      const toolOutcomes = await ToolExecutor.executeAll(tenantId, conversationId, context.workspacePath, parsedCalls);
    ...
    await ToolExecutor.executeSingle(tenantId, conversationId, context.workspacePath, "write_file", {
      path: mockFilePath,
  ...
  private static async runReviewerAgent(task: string, context: Record<string, string>): Promise<string> {
    const systemPrompt = "You are the Harikson Reviewer. Evaluate the changes. If there are syntax or logic errors, reply with 'REJECT' and details. Otherwise reply with 'APPROVE'.";
  ...
  private static async runTesterAgent(
    ...
    const outcome = await ToolExecutor.executeSingle(tenantId, conversationId, context.workspacePath, "run_tests", {});
