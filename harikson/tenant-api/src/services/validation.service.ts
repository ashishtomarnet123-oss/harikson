export class ValidationService {
  ...
  static validateChat(text: string): { isValid: boolean; reason?: string } {
    ...
    if (emailRegex.test(text)) {
      return { isValid: false, reason: "PII Leak: Found email address in generation." };
    }
    if (phoneRegex.test(text)) {
      return { isValid: false, reason: "PII Leak: Found potential phone number." };
    }
    ...
    for (const word of toxicityKeywords) {
      if (lowercaseText.includes(word)) {
        return { isValid: false, reason: `Toxicity Check failed: Found blocked terms.` };
      }
    }
  ...
  static validateCode(code: string, language: string): { isValid: boolean; reason?: string } {
    ...
    for (const pattern of sqliPatterns) {
      if (pattern.test(code)) {
        return { isValid: false, reason: "Security Scan: Found potential SQL Injection exploit query." };
      }
    }
    ...
    for (const pattern of xssPatterns) {
      if (pattern.test(code)) {
        return { isValid: false, reason: "Security Scan: Found potential Cross-Site Scripting (XSS) payload." };
      }
    }
    ...
    if (language === "javascript" || language === "json") {
      try {
        ...
        return { isValid: false, reason: `Syntax Parse failed: ${err.message}` };
      }
    }
