...
export class HariksonScheduler {
  ...
  static startAll(tenantId: string, workspacePath: string, userId: string) {
    Logger.info('🔋 [Harikson Scheduler] Starting background workers...', {
      workspacePath,
    });
  ...
  private static async executeQuery<T>(
    ...
    try {
      ...
      try {
        ...
        console.warn(
          'Warning clearing tenant context on query error:',
          cleanupErr.message
        );
      }
      throw err;
      ...
      const watcher = fs.watch(
        ...
        (eventType, filename) => {
          ...
          setTimeout(async () => {
            ...
            try {
              Logger.info(
                `🔍 [Indexer Worker] File change detected: ${filename}. Re-indexing...`
              );
  ...
  private static async runMemoryExtractorWorker(
    tenantId: string,
    userId: string
  ) {
    await this.executeQuery(tenantId, async (client) => {
      ...
      for (const row of res.rows) {
        ...
        Logger.info(
          `🧠 [Memory Worker] Extracting facts from message ${row.id}...`
        );
  ...
  private static async runConversationSummarizerWorker(
    tenantId: string,
    userId: string
  ) {
    await this.executeQuery(tenantId, async (client) => {
      ...
      for (const row of res.rows) {
        ...
        if (checkRes.rows.length === 0) {
          Logger.info(
            `📝 [Summarizer Worker] Compiling summary for conversation ${conversationId} (>50 messages)...`
          );
  ...
  private static async runCacheWarmerWorker(
    tenantId: string,
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          ...
          try {
            ...
            console.warn(
              `Warning warming cache for file ${full}:`,
              err.message
            );
          }
        }
