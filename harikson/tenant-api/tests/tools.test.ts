...
async function runToolTests() {
  ...
  try {
    ...
    if (parsedCalls.length === 1 && parsedCalls[0].name === "read_file" && parsedCalls[0].params.path === "src/app.js" && parsedCalls[0].params.startLine === 10) {
      console.log("✅ Pass: XML tool tags parsed and converted to typed params successfully.");
    } else {
      throw new Error(`Fail: XML parsing failed. Result: ${JSON.stringify(parsedCalls)}`);
    }
  } catch (err: any) {
  ...
  try {
    ...
    if (exists && fs.readFileSync(createdFilePath, "utf-8").includes("test")) {
      console.log("✅ Pass: write_file executed successfully and created the file.");
    } else {
  ...
  try {
    ...
    console.log(`👉 Outcome status: ${outcome.error ? "Blocked (Error)" : "Passed (Output)"}`);
    
    if (outcome.error && outcome.error.includes("Security Exception")) {
      console.log("✅ Pass: Dangerous command 'rm -rf /' was successfully blocked.");
    } else {
  ...
  try {
    ...
    if (mockDbLogs.some(log => log.tool_name === "write_file" && log.status === "success") &&
        mockDbLogs.some(log => log.tool_name === "run_terminal" && log.status === "error")) {
      console.log("✅ Pass: Tool execution history successfully logged and tracked.");
    } else {
      throw new Error("Fail: Logging did not capture correct execution status entries.");
    }
  } catch (err: any) {
