import { HariksonOrchestrator, TaskStep } from "../src/services/agents/orchestrator.js";
...
async function runAgentTests() {
  console.log("🧪 [Harikson Agent Orchestrator Test Suite] Initializing tests...");
  ...
  OllamaClient.generate = async (prompt, systemPrompt) => {
    if (systemPrompt?.includes("Planner")) {
      return JSON.stringify([
        { step: 1, agent: "Harikson Research", task: "Look for auth structures in workspace." },
        { step: 2, agent: "Harikson Coder", task: "Write JWT middleware file." },
  ...
  (ToolExecutor as any).executeQuery = async (tenantId: string, callback: any) => {
    const mockClient = {
      query: async () => ({ rows: [] })
    };
  ...
  (HariksonOrchestrator as any).executeQuery = async (tenantId: string, callback: any) => {
    const mockClient = {
  ...
  try {
    ...
    console.log(`👉 Execution completed. Status: ${mockPlansDb.get(result.planId)?.status}`);
    ...
    console.log(`👉 Reviewer step result: ${result.steps.find(s => s.agent === "Harikson Reviewer")?.result}`);

    if (coderStep && coderStep.status === "completed" && mockPlansDb.get(result.planId)?.status === "completed") {
      console.log("✅ Pass: Multi-agent execution completed and recovered from loopback successfully.");
    } else {
      throw new Error("Fail: Agent execution loop failed or did not recover from Reviewer reject.");
    }
  } catch (err: any) {
