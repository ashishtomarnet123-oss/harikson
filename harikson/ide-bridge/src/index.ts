...
io.on("connection", (socket) => {
  ...
  socket.on("autocomplete", async (data: { prefix: string; suffix: string; language: string }, callback) => {
    console.log(`💻 [IDE Bridge] Auto-completion request for language: ${data.language}`);
    
    // Simulate low latency code completion response (<300ms)
    setTimeout(() => {
      let completion = "";
      if (data.prefix.trim().endsWith("function")) {
        completion = " calculateMetrics(data) {\n  const cores = data.cores || 1;\n  return cores * 100;\n}";
  ...
  socket.on("sync-workspace", (data: { filesList: string[] }) => {
    console.log(`💻 [IDE Bridge] Indexed local workspace containing ${data.filesList.length} files.`);
  });
