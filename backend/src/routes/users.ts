...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response, next) => {
    try {
      const userId = req.user!.userId;
      const { theme, density, sidebarState, fontSize, accentColor, animation } =
        req.body;
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res: Response, next) => {
    try {
      ...
      res.status(200).json({
        message: 'User approved and instance container provisioned.',
        instance,
      });
    } catch (error) {
      next(error);
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res: Response, next) => {
    try {
      ...
      res
        .status(200)
        .json({ message: 'User suspended and containers halted.' });
    } catch (error) {
      next(error);
    }
  }
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res: Response, next) => {
    try {
      ...
      res
        .status(200)
        .json({ message: 'User unsuspended and workloads resumed.' });
    } catch (error) {
      next(error);
    }
  }
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res: Response, next) => {
    try {
      ...
      res
        .status(200)
        .json({ message: 'User account deleted and all workloads destroyed.' });
    } catch (error) {
      next(error);
    }
  }
