    ...
    vscode.commands.registerCommand('harikson.reviewSelection', async () => {
      ...
      if (!editor) {
        vscode.window.showWarningMessage(
          'No active editor found to review selection.'
        );
        return;
      }
      ...
      if (!selectedText.trim()) {
        vscode.window.showInformationMessage(
          'Please select a code block to review first.'
        );
        return;
      }
        ...
        async (progress) => {
          try {
            ...
            const originalTempPath = path.join(
              workspaceFolder,
              '.harikson-review-original.tmp'
            );
            const reviewedTempPath = path.join(
              workspaceFolder,
              '.harikson-review-suggested.tmp'
            );
            ...
            setTimeout(async () => {
              try {
                ...
                console.warn(
                  'Warning deleting temp code review files:',
                  err.message
                );
              }
            }, 15000);
            ...
            vscode.window.showErrorMessage(
              `Code review failed: ${err.message || err}`
            );
          }
        }
      );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('harikson.switchModel', async () => {
      ...
      if (chosen) {
        ...
        vscode.window.showInformationMessage(
          `Switched active model to ${chosen}`
        );
        checkConnection();
      }
    })
