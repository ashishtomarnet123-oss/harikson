...
function getBrandedModelName(internalName: string): string {
  return (
    HARIKSON_MODELS_BRANDING[internalName]?.displayName ||
    internalName ||
    'Harikson AI'
  );
}
...
export default function HariksonClientDashboard() {
  const [activeSubTab, setActiveSubTab] = useState<
    'playground' | 'widget' | 'rag' | 'finetune' | 'leads'
  >('playground');
  const [agentName, setAgentName] = useState('Harikson');
  const [messages, setMessages] = useState<Message[]>([
    {
      sender: 'bot',
      text: 'Welcome to Harikson AI Console! How can I assist you with your operations today?',
    },
  ]);
  ...
  const loadData = async () => {
    try {
      ...
      if (list && list.length > 0) {
        ...
        const aiInst =
          list.find((inst) => inst.apps?.includes('openwebui')) || list[0];
        setInstance(aiInst);
        setAgentName(
          aiInst.name.charAt(0).toUpperCase() + aiInst.name.slice(1)
        );
      }
      ...
      setDocuments([
        ...
        {
          id: 'doc_2',
          name: 'https://docs.harikson.yourdomain.com/features',
          type: 'URL',
          size: 'Crawled Website',
          status: 'INDEXED',
      ...
      console.warn(
        'Error fetching data, using high-fidelity local fallbacks.',
        err
      );
    }
  };
  ...
  useEffect(() => {
    setMessages([
      {
        sender: 'bot',
        text: `Welcome to ${agentName} AI Console! How can I assist you with your operations today?`,
      },
    ]);
  }, [agentName]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn(
        'Tenant API call failed, falling back to simulated response.',
        err
      );
      // Fallback simulator if the backend container is unreachable
      let botResponse =
        "I've reviewed your request. Based on the RAG index data, we suggest following the standard deployment sequence.";
      if (userMsg.text.toLowerCase().includes('pricing')) {
        botResponse =
          'Our plans start at $29/mo (Starter) and go up to $129/mo (Business). Let me know if you want to update your billing parameters!';
      } else if (
        userMsg.text.toLowerCase().includes('hello') ||
        userMsg.text.toLowerCase().includes('hi')
      ) {
      ...
      >
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: '700' }}>
            {agentName} AI Agents Hub
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.9rem' }}>
            Configure model weights, customize chat widgets, and train adapters
          </p>
        </div>
        <div
        ...
        >
          ...
          >
            ...
            <div className="glass-card" style={{ padding: '24px' }}>
              ...
              >
                Your AI agent answers user queries based on files indexed in the
                RAG repository. Keep your knowledge base updated by uploading
                documentation under the "Knowledge Base" tab.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="glass-card" style={{ padding: '30px' }}>
            ...
            >
              ...
              >
                ...
                >
                  Paste this HTML script tag inside your website body to run the
                  live chat widget:
                </p>
                <div
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    File path name or URL crawler endpoint
                  </label>
                  <input
                    type="text"
                    className="input-field"
                ...
                {documents.map((d) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 10px' }}>
                      <span
                        className={`badge badge-${d.status === 'INDEXED' ? 'running' : 'pending'}`}
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        Base: {getBrandedModelName(log.baseModel)} | Triggered:{' '}
                        {log.createdAt}
                      </p>
                    </div>
                    <span
                      className={`badge badge-${log.status === 'COMPLETED' ? 'running' : 'pending'}`}
                      style={{ fontSize: '0.7rem' }}
                    >
                      {log.status}
                    </span>
          ...
          <div className="glass-card" style={{ padding: '24px' }}>
            ...
            >
              ...
              >
                ...
                />
                <p>
                  Training runs adapt your chatbot's language patterns using
                  your past tickets, logs, and indexing databases. Ensure to
                  verify toxicity scoring prior to adaptions.
                </p>
              </div>
              <div
                style={{
              ...
              >
                <Info
                  size={16}
                  style={{ color: '#3b82f6', marginTop: '2px', flexShrink: 0 }}
                />
                <p>
                  QLoRA fine-tuning runs execute inside isolated GPU clusters.
                  Each training epoch takes roughly 5 to 10 minutes depending on
                  data size.
                </p>
              </div>
            </div>
