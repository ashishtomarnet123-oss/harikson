...
async function runTests() {
  ...
  try {
    ...
    assert.strictEqual(rawToken.length, 64, 'Raw token should be 64 characters long (32 bytes hex)');
    console.log('   ✓ Generated raw token (entropy: 32 bytes hex, length 64):', rawToken.substring(0, 10) + '...');

    const tokenHash = crypto.createHash('sha256').update(rawToken).digest('hex');
    assert.strictEqual(tokenHash.length, 64, 'Token hash should be 64 characters long (SHA-256)');
    console.log('   ✓ Computed SHA-256 hash:', tokenHash.substring(0, 10) + '...');
    ...
    const dbRecord = await pool.query('SELECT token FROM refresh_tokens WHERE user_id = $1', [userId]);
    assert.strictEqual(dbRecord.rows.length, 1);
    assert.strictEqual(dbRecord.rows[0].token, tokenHash, 'Stored token must exactly match computed SHA-256 hash');
    assert.notStrictEqual(dbRecord.rows[0].token, rawToken, 'Stored token must NOT match raw plaintext token');
    console.log('   ✓ Verified: DB contains only the SHA-256 hash, raw token is never written.');

    // 3. Test verification/lookup using hashed value
    console.log('\n🔹 Test 2: Verify lookup using hashed token...');
    const incomingToken = rawToken;
    const lookupHash = crypto.createHash('sha256').update(incomingToken).digest('hex');

    const rtQuery = await pool.query(
      'SELECT * FROM refresh_tokens WHERE token = $1 AND revoked_at IS NULL AND expires_at > NOW() LIMIT 1',
      [lookupHash]
    );
    assert.strictEqual(rtQuery.rows.length, 1, 'Lookup by hash should return the token record');
    ...
    const logoutHash = crypto.createHash('sha256').update(logoutToken).digest('hex');
    ...
    assert.strictEqual(rtQueryRevoked.rows.length, 0, 'Revoked token should not be returned by active query');
    console.log('   ✓ Revocation on logout successfully marked revoked_at and blocked active queries.');
