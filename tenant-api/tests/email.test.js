...
async function runTests() {
  try {
    ...
    const res3 = await sendInvoiceReceipt(testEmail, {
      amount: 1000,
      currency: 'USD',
      status: 'paid',
    });
    ...
    assert.ok(
      res4.error.includes('Rate limit exceeded'),
      'Should trigger rate limit error'
    );
    ...
    console.log(
      '   ✓ Email call failed gracefully as expected: ' + badRes.error
    );
