...
import { HardDrive, Plus, Check, AlertTriangle, RefreshCw, Shield, Database, Calendar } from 'lucide-react';
...
interface BackupStats { completed: string; failed: string; total_bytes: string; }

const STATUS_CONFIG: Record<string, { color: string; bg: string; text: string }> = {
  pending: { color: 'yellow', bg: 'bg-yellow-500/10 border-yellow-500/30', text: 'text-yellow-400' },
  running: { color: 'blue', bg: 'bg-blue-500/10 border-blue-500/30', text: 'text-blue-400' },
  completed: { color: 'emerald', bg: 'bg-emerald-500/10 border-emerald-500/30', text: 'text-emerald-400' },
  failed: { color: 'red', bg: 'bg-red-500/10 border-red-500/30', text: 'text-red-400' },
  verified: { color: 'indigo', bg: 'bg-indigo-500/10 border-indigo-500/30', text: 'text-indigo-400' },
};
  ...
  const triggerBackup = async (type: string = 'full') => {
    ...
    try {
      await fetch(`${apiBase}/admin/backups`, {
        ...
        body: JSON.stringify({ name, type, retention_days: type === 'full' ? 30 : 7 }),
      });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <HardDrive className="w-6 h-6 text-amber-500" /> Backup & Disaster Recovery
          </h1>
          <p className="text-gray-400 mt-1 text-sm">Manage full and incremental database backups with retention policies.</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={fetchBackups} className="p-2 hover:bg-gray-800 rounded-lg text-gray-400 transition-colors">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-gray-900/60 border-blue-900/30 p-4 text-center">
          <div className="text-3xl font-black text-blue-400">{fmtBytes(totalBytes)}</div>
          <div className="text-xs text-gray-500 mt-1">Total Storage</div>
        </Card>
        <Card className={`border p-4 text-center ${failedCount > 0 ? 'bg-red-950/20 border-red-900/30' : 'bg-gray-900/60 border-gray-800'}`}>
          <div className={`text-3xl font-black ${failedCount > 0 ? 'text-red-400' : 'text-gray-600'}`}>{failedCount}</div>
      ...
      {runningCount > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 bg-blue-950/30 border border-blue-800/40 rounded-xl text-blue-300 text-sm font-medium">
          ...
          {runningCount} backup{runningCount > 1 ? 's' : ''} currently running — auto-refreshing...
        </div>
      )}
      ...
      <Card className="bg-gray-900/40 border-gray-800 p-0 overflow-hidden">
        <div className="px-5 py-3.5 border-b border-gray-800 flex items-center justify-between">
          <span className="text-sm font-semibold text-gray-300">Backup History</span>
          {backups.length > 0 && <span className="text-xs text-gray-500">{backups.length} records</span>}
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-900/50">
                {['Name', 'Type', 'Size', 'Status', 'Started', 'Completed', 'Retention', 'Actions'].map(h => (
            ...
            <tbody>
              ...
              {!loading && backups.length === 0 && (
                <tr><td colSpan={8} className="text-center py-12">
                  ...
                  <div className="text-gray-400 font-medium mb-1">No backups yet</div>
                  <div className="text-gray-600 text-xs">Click "Full Backup" above to create your first backup.</div>
                </td></tr>
              )}
              {backups.map(b => {
                ...
                return (
                  <tr key={b.id} className="border-b border-gray-800/40 hover:bg-gray-800/20 transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-gray-300 max-w-[180px] truncate" title={b.name}>{b.name}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${b.type === 'full' ? 'bg-amber-500/10 text-amber-400' : 'bg-blue-500/10 text-blue-400'}`}>
                        {b.type}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-400 font-mono text-xs">{fmtBytes(b.size_bytes)}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-md border ${sc.bg} ${sc.text}`}>
                        {b.status === 'running' && <RefreshCw className="w-3 h-3 inline mr-1 animate-spin" />}
                        {b.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{fmtTime(b.started_at)}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{fmtTime(b.completed_at)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 text-gray-400 text-xs">
                        <Calendar className="w-3 h-3" />{b.retention_days}d
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        {b.status === 'completed' && (
                          <button
                            ...
                            {verifying === b.id ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Shield className="w-3 h-3" />}
                            Verify
                          </button>
                        )}
                        {b.status === 'verified' && (
