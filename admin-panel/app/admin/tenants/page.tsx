...
function PlanEditModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div
        className="w-full max-w-2xl bg-gray-900 border border-gray-800 rounded-2xl flex flex-col overflow-hidden shadow-2xl"
      ...
      >
        ...
        <div className="flex-1 min-h-0 px-6 py-5 overflow-y-auto space-y-5 text-sm text-gray-300">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5 font-semibold">
                Plan Name
              </label>
              <input
                value={draft.name}
                onChange={(e) => update('name', e.target.value)}
                className="w-full bg-gray-950 border border-gray-800 text-xs rounded-xl p-2.5 text-white outline-none focus:border-indigo-500"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5 font-semibold">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5 font-semibold">
                Token Limit/month (-1 = unlimited)
              </label>
              <input
                type="number"
                value={draft.tokenLimit}
                onChange={(e) => update('tokenLimit', Number(e.target.value))}
                className="w-full bg-gray-950 border border-gray-800 text-xs rounded-xl p-2.5 text-white outline-none focus:border-indigo-500"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5 font-semibold">
                Tenant Limit (-1 = unlimited)
              </label>
              <input
                type="number"
                value={draft.tenantLimit}
                onChange={(e) => update('tenantLimit', Number(e.target.value))}
                className="w-full bg-gray-950 border border-gray-800 text-xs rounded-xl p-2.5 text-white outline-none focus:border-indigo-500"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5 font-semibold">
                Agent Limit (-1 = unlimited)
              </label>
              <input
                type="number"
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-gray-950 border border-gray-800 text-xs rounded-xl p-2.5 text-white outline-none focus:border-indigo-500 resize-y"
            />
          </div>
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {draft.features.map((f) => {
                ...
                return (
                  <div
                    key={f.key}
                    className="flex items-center justify-between p-3 bg-gray-950/40 border border-gray-800/60 rounded-xl"
                  >
                    ...
                    {f.type === 'boolean' ? (
                      <button
                        type="button"
                        onClick={() => updateFeature(f.key, !f.value)}
                        className={`text-2xl transition ${f.value ? 'text-indigo-400' : 'text-gray-600'}`}
                      >
                        {f.value ? (
                          <ToggleRight size={24} />
                        ) : (
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateFeature(f.key, Number(e.target.value))
                        }
        ...
        <div className="px-6 py-4 border-t border-gray-800 bg-gray-950/20 flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-gray-850 hover:bg-gray-800 border border-gray-800 rounded-xl text-xs font-semibold text-gray-400 transition"
          >
            Cancel
          </button>
          <button
...
export default function TenantPlanManager() {
  ...
  const [editingTenantPlan, setEditingTenantPlan] = useState<Tenant | null>(
    null
  );
  ...
  const [editingTenantDetails, setEditingTenantDetails] =
  ...
  const [providersActive, setProvidersActive] = useState<{
    razorpay: boolean;
    stripe: boolean;
  }>({ razorpay: false, stripe: false });
  const [providersModes, setProvidersModes] = useState<{
    razorpay: string;
    stripe: string;
  }>({ razorpay: 'test', stripe: 'test' });
  ...
  const fetchData = async () => {
    ...
    try {
          ...
          features: [
            {
              ...
              label: 'API Access',
              type: 'boolean',
              value: p.features?.api_access !== false,
              icon: Code2,
            },
            {
              key: 'webhook_logging',
              label: 'Webhook Logging',
              type: 'boolean',
              value: !!p.features?.webhook_logging,
              icon: Webhook,
            },
            {
              key: 'rag_documents',
              ...
              type: 'number',
              value:
                typeof p.features?.rag_documents === 'number'
                  ? p.features.rag_documents
                  : 500,
              icon: Database,
            },
            {
            ...
            },
            {
              ...
              label: 'Priority Support',
              type: 'boolean',
              value: !!p.features?.priority_support,
              icon: Star,
            },
            {
              key: 'custom_models',
              label: 'Custom Model Fine-Tuning',
              type: 'boolean',
              value: !!p.features?.custom_models,
              icon: Cpu,
            },
            {
              key: 'dpdp_compliance',
              label: 'DPDP Compliance',
              type: 'boolean',
              value: p.features?.dpdp_compliance !== false,
              icon: Lock,
            },
            {
              key: 'sla_hours',
              ...
              type: 'number',
              value:
                typeof p.features?.sla_hours === 'number'
                  ? p.features.sla_hours
                  : 72,
              icon: Clock,
            },
          ],
      ...
      if (resTenants.status === 'fulfilled' && resTenants.value.ok) {
        const d = await resTenants.value.json();
        setTenants(d.tenants || []);
      }
      if (resViolations.status === 'fulfilled' && resViolations.value.ok) {
        const d = await resViolations.value.json();
        setViolations(d.violations || []);
      }
      if (resReconcile.status === 'fulfilled' && resReconcile.value.ok) {
        const d = await resReconcile.value.json();
        setReconcile(d.billing || []);
      }
      if (resKeys.status === 'fulfilled' && resKeys.value.ok) {
        const d = await resKeys.value.json();
        setApiKeys(d.keys || []);
      }
  ...
  const handleToggleSuspend = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      showToast(
        `Tenant ${targetStatus === 'active' ? 'activated' : 'suspended'}`
      );
    } catch {
      showToast('Failed to change tenant status.', 'error');
    }
  };
  ...
  const setRecommended = async (id: string) => {
    ...
    try {
      ...
      setPlans((prev) =>
        prev.map((p) => ({ ...p, isRecommended: p.id === id }))
      );
  ...
  const handleGenerateKey = async (e: React.FormEvent) => {
    ...
    if (!newKeyTenant || !newKeyName) {
      showToast('Tenant and Key Name are required', 'error');
      return;
    }
    const token =
      getCookie('admin_token') ||
  ...
  const handleRevokeKey = async (id: string) => {
    if (
      !window.confirm(
        'Permanently revoke this API key? Apps using it will stop working.'
      )
    )
      return;
    const token =
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm ml-13">
            Tenants, plans, API keys, and payment webhooks — all in one place.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          return (
            <div
              key={s.label}
              className="bg-gray-900/40 border border-gray-800/80 rounded-2xl p-5 flex items-center gap-4"
            >
              <div
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-[10px] text-gray-500 mt-0.5">
                    Prompt in vs. generation out — last 7 days
                  </p>
                </div>
            ...
            <div className="bg-gray-900/40 border border-gray-800/80 p-6 rounded-2xl">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-[10px] text-gray-500 mt-0.5">
                    Active consumption this billing cycle
                  </p>
                </div>
            ...
            <div className="p-5 border-b border-gray-800 flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Manage tenant configurations, token caps, and subscriptions.
                </p>
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-4 px-6">
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${t.status === 'active' ? 'bg-green-500' : 'bg-red-500'}`}
                            />
                            {t.status}
                          </span>
                        </td>
                        <td className="py-4 px-6 text-right space-x-2">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 bg-gray-850 hover:bg-gray-800 border border-gray-800 rounded text-xs font-semibold text-indigo-400"
                          >
                            Edit Plan
                          </button>
                          <button
          ...
          <section className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div className="bg-gray-900/40 border border-gray-800/80 rounded-2xl overflow-hidden">
              <div className="p-5 border-b border-gray-800 flex justify-between items-center gap-4">
                <div>
                  ...
                  <p className="text-[10px] text-gray-500 mt-0.5">
                    Logs of throttled client requests
                  </p>
                </div>
                <input
                  type="text"
            ...
            <div className="bg-gray-900/40 border border-gray-800/80 rounded-2xl overflow-hidden">
              <div className="p-5 border-b border-gray-800">
                ...
                <p className="text-[10px] text-gray-500 mt-0.5">
                  Razorpay transactions vs credited tokens
                </p>
              </div>
              <div className="overflow-x-auto">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">
                Define pricing tiers, feature access, and model permissions for
                all tenants.
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              return (
                ...
                >
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center bg-gray-950/40"
                    style={{ color: s.color }}
                  >
                    <Icon size={18} />
                  </div>
                ...
                >
                  <div className="p-5 flex items-center gap-5 flex-wrap md:flex-nowrap">
                    ...
                    <div className="text-right shrink-0 min-w-[130px]">
                      {plan.billing === 'custom' ? (
                        <div className="text-base font-bold text-purple-400 font-sans">
                          Custom Pricing
                        </div>
                      ) : (
                          ...
                          <div className="space-y-2">
                            {plan.features.map((f) => {
                              ...
                              return (
                                <div
                                  key={f.key}
                                  className="flex items-center justify-between text-xs py-1.5 border-b border-gray-850 last:border-0"
                                >
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5 mb-4">
                            {plan.modelAccess.map((m) => (
                              <span
                                key={m}
                                className="text-[10px] px-2.5 py-1 rounded-full font-medium tracking-wide bg-gray-950 border border-gray-850 text-gray-300"
                              >
                                {m}
                              </span>
                            ))}
                          ...
                          <div className="space-y-2">
                            {[
                              ...
                              },
                              {
                                ...
                                value:
                                  plan.billing.charAt(0).toUpperCase() +
                                  plan.billing.slice(1),
                              },
                              {
                                label: 'Active Subscribers',
                                value: `${subCount} active tenant${subCount !== 1 ? 's' : ''}`,
                              },
                            ].map((row) => (
                              <div
                                key={row.label}
                                className="flex justify-between text-xs py-1 border-b border-gray-850 last:border-0"
                              >
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Generate and revoke programmatic access credentials per tenant.
            </p>
          </div>
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="bg-gray-900/40 border border-gray-800/80 rounded-2xl p-6">
              ...
              <p className="text-[10px] text-gray-500 mb-5">
                Create isolated credentials linked to a tenant with specific
                rate limits.
              </p>
            ...
            <div className="bg-gray-900/40 border border-gray-800/80 rounded-2xl overflow-hidden xl:col-span-2">
              <div className="p-5 border-b border-gray-800">
                ...
                <p className="text-[10px] text-gray-500 mt-0.5">
                  Programmatic API integration keys with strict token budgets.
                </p>
              </div>
              <div className="overflow-x-auto">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-500 text-sm">
                Real-time Stripe and Razorpay webhook audit stream.
              </p>
            </div>
            <a
              href="/admin/billing/providers"
              className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl transition-all"
            >
              Configure Providers
            </a>
          </div>
          ...
          <div className="bg-gray-900/40 border border-gray-800/80 rounded-2xl p-5 flex gap-8">
            {[
              {
                name: 'Razorpay',
                active: providersActive.razorpay,
                mode: providersModes.razorpay,
                color: 'text-blue-400',
              },
              {
                name: 'Stripe',
                active: providersActive.stripe,
                mode: providersModes.stripe,
              ...
              <div key={p.name} className="flex items-center gap-3">
                ...
                {p.active ? (
                  <span
                    className={`${p.color} text-xs font-bold flex items-center gap-1`}
                  >
                ...
                <tbody className="divide-y divide-gray-800 text-gray-300">
                  ...
                  {webhooks.map((wh) => (
                    <tr key={wh.id} className="hover:bg-gray-800/10">
                      <td
                        className="py-3 px-4 font-bold text-white text-[11px] truncate max-w-[160px]"
                        title={wh.event_type}
                      >
                        {wh.event_type}
                      </td>
                      <td className="py-3 px-4">
                        <span
                          className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${wh.provider === 'stripe' ? 'bg-purple-900/30 text-purple-400' : 'bg-blue-900/30 text-blue-400'}`}
                        >
                          {wh.provider}
                        </span>
                      </td>
      ...
      {selectedTenant && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-gray-900 border-l border-gray-800 p-6 flex flex-col justify-between overflow-y-auto">
            <div>
              ...
              <div className="space-y-4 text-sm">
                {[
                  ...
                  {
                    label: 'Active Users',
                    value: String(selectedTenant.user_count),
                  ...
                  <div
                    key={row.label}
                    className="p-3 bg-gray-950/50 rounded-xl border border-gray-800"
                  >
                  ...
                  {selectedTenant.price !== undefined && (
                    <div className="border-t border-gray-850 pt-2.5 grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div>
                        ...
                        <span className="text-gray-300">
                          {selectedTenant.agent_limit === -1
                            ? 'Unlimited'
                            : selectedTenant.agent_limit}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500 block">SLA Target</span>
                        <span className="text-gray-300">
                          {selectedTenant.features?.sla_hours
                            ? `${selectedTenant.features.sla_hours} hrs`
                            : 'N/A'}
                        </span>
                      </div>
                    </div>
                  )}
                  {selectedTenant.model_access &&
                    selectedTenant.model_access.length > 0 && (
                ...
                <button
                  onClick={() => {
                    handleOpenEditDetails(selectedTenant);
                ...
                <button
                  onClick={() => {
                    setEditingTenantPlan(selectedTenant);
                    setSelectedTenant(null);
                  }}
            ...
            <button
              onClick={() => setSelectedTenant(null)}
              className="w-full py-2.5 bg-gray-850 hover:bg-gray-800 border border-gray-800 text-sm font-semibold rounded-xl mt-4"
            >
              Close
            </button>
          </div>
          ...
          <div className="w-full max-w-md bg-gray-900 border border-gray-800 p-6 rounded-2xl flex flex-col gap-4 text-sm text-gray-300">
            <div className="flex justify-between items-center mb-2">
              ...
              <button
                ...
                className="p-1 hover:bg-gray-850 rounded"
              >
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>
            <p>
              Select a new subscription plan for{' '}
              <strong>{editingTenantPlan.name}</strong>:
            </p>
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${
                      p.tier === 'starter'
                        ...
                        : p.tier === 'professional'
                          ? 'bg-indigo-900/40 text-indigo-400 border border-indigo-800/30'
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-md bg-gray-900 border border-gray-800 p-6 rounded-2xl shadow-2xl">
            ...
            <div className="p-3 bg-gray-950 rounded-xl border border-gray-850 flex items-center justify-between gap-3 mb-5">
              <span className="font-mono text-xs text-indigo-400 break-all select-all font-semibold">
                {generatedPlainKey}
              </span>
              <button
                onClick={() => {
      ...
      {selectedWebhook && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div
            className="w-full max-w-lg bg-gray-900 border border-gray-800 p-6 rounded-2xl shadow-2xl flex flex-col"
            style={{ maxHeight: '85vh' }}
          >
            ...
            <div className="flex-1 min-h-0 space-y-4 mb-5 text-xs text-gray-300 overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  },
                  {
                    ...
                    value: (
                      <span
                        className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${selectedWebhook.provider === 'stripe' ? 'bg-purple-900/30 text-purple-400' : 'bg-blue-900/30 text-blue-400'}`}
                      >
                  ...
                  },
                  {
                    ...
                    value: (
                      <span
                        className={
                          selectedWebhook.signature_verified
                  ...
                  <div
                    ...
                    className="p-3 bg-gray-950/50 rounded-xl border border-gray-800"
                  >
                    <span className="text-[10px] text-gray-500 uppercase block font-bold mb-1">
                      {row.label}
                    </span>
                    {row.value}
                  </div>
              ...
              <div className="flex gap-4">
                <button
                  type="button"
                  onClick={() => showToast('Redelivery triggered')}
                ...
                <button
                  type="button"
                  onClick={() => {
                    showToast('Event marked as processed');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-gray-900 border border-gray-800 p-6 rounded-2xl max-w-sm w-full shadow-2xl">
            ...
            <p className="text-xs text-gray-400 leading-relaxed mb-6">
              ...
              <strong>{plans.find((p) => p.id === deleteConfirm)?.name}</strong>
              ? This cannot be undone and may affect{' '}
              {subscriberCount(deleteConfirm)} active subscriber
              {subscriberCount(deleteConfirm) !== 1 ? 's' : ''}.
            </p>
