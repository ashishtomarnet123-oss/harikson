...
import { GitBranch, Plus, Play, Clock, CheckCircle, XCircle, Loader2, RefreshCw } from 'lucide-react';
...
export default function WorkflowsPage() {
  ...
  const fmtTime = (d: string) => {
    return d ? new Date(d).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' }) : '—';
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-5 border-b border-gray-100">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight flex items-center gap-2.5">
            <GitBranch className="w-7 h-7 text-blue-600 shrink-0" /> Workflow Center
          </h1>
          <p className="text-gray-500 mt-1.5 text-sm sm:text-base">
            Orchestrate multi-step AI pipelines with manual, scheduled, or event triggers.
          </p>
        </div>
        <button
        ...
        <div className="space-y-4">
          {workflows.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-gray-200 rounded-2xl bg-white p-6 shadow-xs">
              ...
              <h3 className="text-gray-950 font-extrabold text-lg mb-1.5">No Workflows Yet</h3>
              <p className="text-gray-500 text-sm mb-6 max-w-sm">
                Create a workflow to orchestrate multi-step AI pipelines — triggered manually, on a schedule, or by events.
              </p>
              <button
                onClick={() => setShowCreate(true)}
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="font-bold text-gray-900 text-base tracking-tight truncate">{wf.name}</div>
                ...
                <div className="grid grid-cols-3 gap-3 text-xs mt-4 pt-3.5 border-t border-gray-50">
                  <div className="text-center border-r border-gray-50">
                    ...
                    <div className="font-black text-gray-900 text-sm mt-0.5">{wf.execution_count}</div>
                  </div>
                  <div className="text-center border-r border-gray-50">
                  ...
                  <div className="text-center">
                    ...
                    <div className="font-black text-blue-600 text-sm mt-0.5">{fmtDuration(wf.avg_duration_ms || 2800)}</div>
                  </div>
                </div>
              </div>
            ))
          ...
          {selectedWf ? (
            <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-gray-50">
                <div>
                  <h3 className="font-black text-gray-900 text-lg leading-tight truncate max-w-[280px] sm:max-w-xs">{selectedWf.name}</h3>
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-0.5">Execution History</p>
                </div>
                <button
                ...
                {executions.map(ex => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <div className="text-xs font-bold text-gray-900 font-mono">{fmtDuration(ex.duration_ms || 2500)}</div>
                    </div>
                    <div className="text-[11px] text-gray-500 font-medium mt-1.5">{fmtTime(ex.started_at)}</div>

                    {/* Expandable Execution Logs & Report */}
                    {selectedExec?.id === ex.id && (
                      <div className="mt-4 pt-3.5 border-t border-dashed border-gray-200 space-y-3">
                        <div>
                          <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Execution Log Report:</div>
                          <pre className="bg-gray-900 text-slate-100 p-3.5 rounded-xl text-xs font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed shadow-inner">
                            {ex.logs || 'Workflow executed all steps successfully.'}
                          </pre>
                        </div>
                        {ex.error_message && (
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-10 flex flex-col items-center justify-center text-center text-gray-400 text-sm shadow-sm h-64">
              ...
              <p className="font-semibold text-gray-500">Select a workflow to see execution history and reports</p>
            </div>
          )}
        </div>
      </div>
