...
export default function SystemMonitor() {
  ...
  const triggerModelAction = async (endpoint: string, actionName: string, confirmMsg?: string) => {
  ...
  const handleSwitchTo8B = async () => {
    if (!window.confirm('Switch to Qwen3-8B? This will unload the current model.')) return;
  ...
  const getSystemAlerts = () => {
    if (!metrics) return { level: 'green', message: 'All systems operational.' };
    ...
    if (gpuPercent > 90) {
      return { level: 'red', message: 'GPU utilization exceeds 90%! System is highly loaded.' };
    }
    if (metrics.vllm_status !== 'active') {
      return { level: 'red', message: 'vLLM server instance is currently unresponsive or offline.' };
    }
    if (diskPercent > 95) {
      return { level: 'red', message: 'Disk space critical! Utilization exceeds 95%.' };
    }
    if (ramPercent > 85) {
      return { level: 'yellow', message: 'System RAM utilization is high (>85%). Monitor resources closely.' };
    }
    if (metrics.queue > 10) {
      return { level: 'yellow', message: 'Inference request queue depth exceeds 10 waiting logs.' };
    }
  ...
  if (loading && !metrics) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <span className="text-gray-400 font-semibold text-sm">Querying system metrics...</span>
      </div>
    );
  }
  ...
  const vramPercent = metrics ? Math.round((metrics.gpu.used / metrics.gpu.total) * 100) : 0;
  const ramPercent = metrics ? Math.round((metrics.ram.used / metrics.ram.total) * 100) : 0;
  const diskPercent = metrics ? Math.round((metrics.disk.used / metrics.disk.total) * 100) : 0;
    ...
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <div className="bg-gray-900/40 border border-gray-800/80 p-5 rounded-2xl flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">GPU VRAM utilization</span>
            <Database className="w-5 h-5 text-indigo-500" />
          </div>
          <div>
            <div className="text-2xl font-black text-white">{vramUsedGB}GB <span className="text-sm font-medium text-gray-500">/ {vramTotalGB}GB</span></div>
            <div className="w-full bg-gray-950 h-2.5 rounded-full overflow-hidden mt-3">
              <div 
                className={`h-full rounded-full transition-all duration-500 ${
                  vramPercent > 90 ? 'bg-red-500' : vramPercent > 70 ? 'bg-yellow-500' : 'bg-green-500'
                }`}
        ...
        <div className="bg-gray-900/40 border border-gray-800/80 p-5 rounded-2xl flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">System RAM utilization</span>
            <Layers className="w-5 h-5 text-indigo-500" />
          </div>
          <div>
            <div className="text-2xl font-black text-white">{ramUsedGB}GB <span className="text-sm font-medium text-gray-500">/ {ramTotalGB}GB</span></div>
            <div className="w-full bg-gray-950 h-2.5 rounded-full overflow-hidden mt-3">
              <div 
                className={`h-full rounded-full transition-all duration-500 ${
                  ramPercent > 90 ? 'bg-red-500' : ramPercent > 70 ? 'bg-yellow-500' : 'bg-green-500'
                }`}
        ...
        <div className="bg-gray-900/40 border border-gray-800/80 p-5 rounded-2xl flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">CPU workload</span>
            <Cpu className="w-5 h-5 text-indigo-500" />
          </div>
          <div className="flex items-end justify-between gap-4">
            <div className="text-2xl font-black text-white">{metrics?.cpu.percent}%</div>
            {/* SVG Sparkline */}
            <div className="h-10 w-28">
              <svg className="w-full h-full" viewBox="0 0 100 40">
                <polyline
                  ...
                  stroke="#6366f1"
                  strokeWidth="2"
                  points={cpuHistory.map((val, idx) => `${(idx / (cpuHistory.length - 1)) * 100},${40 - (val / 100) * 35}`).join(' ')}
                />
              </svg>
            </div>
          </div>
        ...
        <div className="bg-gray-900/40 border border-gray-800/80 p-5 rounded-2xl flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Disk storage Used</span>
            <Disc className="w-5 h-5 text-indigo-500" />
          </div>
          <div>
            <div className="text-2xl font-black text-white">{metrics?.disk.used}GB <span className="text-sm font-medium text-gray-500">/ {metrics?.disk.total}GB</span></div>
            <div className="w-full bg-gray-950 h-2.5 rounded-full overflow-hidden mt-3">
              <div 
      ...
      <section className="bg-gray-900/40 border border-gray-800/80 p-4 rounded-2xl flex flex-wrap gap-x-12 gap-y-4 items-center justify-between">
        ...
        <div className="flex items-center gap-2.5 text-sm font-semibold">
          ...
          <span className={`inline-flex items-center gap-1.5 font-bold ${
            metrics?.vllm_status === 'active' ? 'text-green-400' : 'text-red-400'
          }`}>
            <span className={`w-2.5 h-2.5 rounded-full ${metrics?.vllm_status === 'active' ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
        ...
        <div className="flex items-center gap-2.5 text-sm font-semibold">
          ...
          <span className="px-2 py-0.5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs rounded-md font-bold uppercase">
            {metrics?.active_model === 'None' ? 'No Model Loaded' : metrics?.active_model}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2.5 text-sm font-semibold">
          ...
          <span className="font-mono text-indigo-400 font-bold">{metrics?.tensor_parallel} GPU</span>
        </div>
      </section>
      ...
      <section className="bg-gray-900/40 border border-gray-800/80 rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-gray-800 flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-black text-white">Qwen Model Control Hub</h2>
            <p className="text-xs text-gray-500 mt-1">Spin up or terminate dedicated vLLM server workers. Only one model can be active.</p>
          </div>

          <div className="flex items-center gap-3">
            <button
              onClick={() => triggerModelAction('unload-all', 'unload-all', 'Emergency unload all models?')}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-800 text-sm">
              {models.map((m) => {
                const isCurrent = metrics?.active_model.toLowerCase().includes(m.id);
                return (
                  <tr key={m.id} className="hover:bg-gray-800/10 text-gray-300 transition-all">
                    <td className="py-4 px-6 font-bold text-white flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${isCurrent ? 'bg-green-500 animate-pulse' : 'bg-gray-700'}`} />
                      {m.name}
                    </td>
                    <td className="py-4 px-6 font-mono text-xs">{m.size}</td>
                    <td className="py-4 px-6 font-mono text-xs">{m.quant}</td>
                    <td className="py-4 px-6 font-mono text-xs text-indigo-400 font-bold">{m.vram}</td>
                    <td className="py-4 px-6 text-gray-500 text-xs">{m.desc}</td>
                    <td className="py-4 px-6 text-right">
                      {isCurrent ? (
                        <button
                          onClick={() => triggerModelAction(`${m.id}/unload`, 'unload', 'Confirm model unload?')}
                          disabled={actionLoading !== null}
                        ...
                        <button
                          onClick={() => triggerModelAction(`${m.id}/load`, 'load')}
                          disabled={actionLoading !== null}
