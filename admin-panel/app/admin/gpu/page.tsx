...
export default function GpuPage() {
  ...
  const getTempColor = (t: number) =>
    t > 85 ? 'text-red-400' : t > 70 ? 'text-yellow-400' : 'text-emerald-400';
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Cpu className="w-6 h-6 text-cyan-500" /> GPU Infrastructure
          </h1>
          <p className="text-gray-400 mt-1 text-sm">
            Live GPU utilization, thermal, and power metrics. Refreshes every 5
            seconds.
          </p>
        </div>
      ...
      {!loading && gpus.length === 0 && (
        <Card className="bg-gray-900/40 border-yellow-800/50 p-8 text-center">
          ...
          <p className="text-gray-500 text-sm">
            {message ||
              'GPU may not be accessible from the current environment, or NVIDIA drivers are not installed.'}
          </p>
        </Card>
      )}
      ...
      {gpus.map((gpu) => (
        <Card key={gpu.index} className="bg-gray-900/40 border-gray-800 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div
              className={`text-2xl font-black ${getUtilColor(gpu.utilization_gpu).replace('bg-', 'text-')}`}
            >
              {gpu.utilization_gpu}%
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <ProgressBar
                ...
                max={100}
                color={getUtilColor(gpu.utilization_gpu)}
              />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-400 flex items-center gap-1">
                  <MemoryStick className="w-3 h-3" /> VRAM
                </span>
                <span className="font-mono text-white">
              ...
              <ProgressBar
                ...
                color={getUtilColor(gpu.utilization_memory)}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <ProgressBar
                ...
                color={
                  gpu.power_draw_w > gpu.power_limit_w * 0.9
                    ? 'bg-red-500'
                    : 'bg-blue-500'
                }
              />
            ...
            <div className="bg-gray-950 rounded-xl p-4 text-center">
              <Thermometer
                className={`w-8 h-8 mx-auto mb-1 ${getTempColor(gpu.temperature_c)}`}
              />
              <div
                className={`text-3xl font-black ${getTempColor(gpu.temperature_c)}`}
              >
                {gpu.temperature_c}°C
              </div>
