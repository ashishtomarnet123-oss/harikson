...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!token) { router.push('/admin/login'); } else {
      setAuthenticated(true);
      const userStr = localStorage.getItem('admin_user');
      if (userStr) { try { const u = JSON.parse(userStr); setAdminEmail(u.email || 'admin@harikson.ai'); } catch { setAdminEmail('admin@harikson.ai'); } }
      setLoading(false);
    }
  }, [router, pathname]);
  ...
  const handleLogout = () => { deleteCookie('admin_token'); localStorage.removeItem('admin_token'); localStorage.removeItem('admin_user'); router.push('/admin/login'); };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex font-sans">
      ...
      <header className="lg:hidden w-full h-16 bg-gray-900 border-b border-gray-800 flex items-center justify-between px-4 fixed top-0 left-0 z-30">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setShowSearch(true)} className="p-1.5 hover:bg-gray-800 rounded-lg"><Search className="w-5 h-5 text-gray-400" /></button>
          <button onClick={() => setShowNotifs(!showNotifs)} className="p-1.5 hover:bg-gray-800 rounded-lg relative">
            <Bell className="w-5 h-5 text-gray-400" />
            {unreadCount > 0 && <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center">{unreadCount}</span>}
          </button>
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-60 bg-gray-900 border-r border-gray-800 flex flex-col justify-between transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="h-14 flex items-center justify-between px-5 border-b border-gray-800 shrink-0">
            <div className="flex items-center gap-2.5">
              <Cpu className="w-6 h-6 text-indigo-500" />
              <span className="font-extrabold tracking-wider text-sm bg-gradient-to-r from-indigo-400 to-purple-500 bg-clip-text text-transparent">HARIKSON</span>
            </div>
            <div className="hidden lg:flex items-center gap-1.5">
              <button onClick={() => setShowSearch(true)} className="p-1.5 hover:bg-gray-800 rounded-lg" title="Search (⌘K)"><Search className="w-4 h-4 text-gray-500" /></button>
              <button onClick={() => setShowNotifs(!showNotifs)} className="p-1.5 hover:bg-gray-800 rounded-lg relative" title="Notifications">
                <Bell className="w-4 h-4 text-gray-500" />
                {unreadCount > 0 && <span className="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-[8px] font-bold rounded-full w-3.5 h-3.5 flex items-center justify-center">{unreadCount}</span>}
              </button>
            </div>
          </div>
          ...
          <div className="px-4 py-3 border-b border-gray-800 shrink-0">
            <div className="flex items-center gap-2.5 px-2 py-1.5 bg-gray-950/50 border border-gray-800/80 rounded-xl">
              <div className="w-7 h-7 rounded-lg bg-indigo-600 flex items-center justify-center font-bold text-white text-xs shrink-0">A</div>
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
            {menuSections.map(section => (
              <div key={section.label}>
                <div className="text-[10px] font-bold text-gray-600 uppercase tracking-widest px-2 mb-1">{section.label}</div>
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col min-w-0 pt-16 lg:pt-0 overflow-hidden">
        ...
        <div className="fixed right-4 top-16 z-50 w-80">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
              ...
              <div className="flex items-center gap-2">
                {unreadCount > 0 && <button onClick={async () => { const token = getCookie('admin_token') || localStorage.getItem('admin_token'); await fetch(`${apiBase}/admin/notifications/read-all`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` } }).catch(()=>{}); fetchNotifications(); }} className="text-xs text-indigo-400 hover:text-indigo-300">Mark all read</button>}
                <button onClick={() => setShowNotifs(false)} className="text-gray-600 hover:text-gray-400"><X className="w-4 h-4" /></button>
              </div>
            </div>
            <div className="max-h-80 overflow-y-auto">
              {notifications.length === 0 && <div className="text-center text-gray-500 text-sm py-8">No notifications</div>}
              {notifications.map(n => (
                <div key={n.id} onClick={() => markRead(n.id)} className={`px-4 py-3 border-b border-gray-800/50 cursor-pointer hover:bg-gray-800/40 transition-colors ${!n.is_read ? 'bg-indigo-950/20' : ''}`}>
                  <div className="flex items-start gap-2">
                    {!n.is_read && <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 mt-1.5 shrink-0" />}
                    <div className={!n.is_read ? '' : 'ml-3.5'}>
                      <div className="text-sm font-medium text-gray-200">{n.title}</div>
                      <div className="text-xs text-gray-500 mt-0.5">{n.message}</div>
                      <div className="text-xs text-gray-700 mt-1">{new Date(n.created_at).toLocaleTimeString()}</div>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* Cmd+K Search Modal */}
      {showSearch && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-24 px-4" onClick={() => { setShowSearch(false); setSearchQuery(''); setSearchResults([]); }}>
          <div className="w-full max-w-lg bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-800">
              ...
              <kbd className="text-xs bg-gray-800 text-gray-500 px-1.5 py-0.5 rounded font-mono">ESC</kbd>
            </div>
            {searchResults.length > 0 && (
              <div className="max-h-72 overflow-y-auto py-2">
                {searchResults.map(r => (
                  <Link key={r.id} href={`${resultLinks[r.type] || '/admin'}?id=${r.id}`}
                    onClick={() => { setShowSearch(false); setSearchQuery(''); setSearchResults([]); }}
                    className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-800/60 transition-colors cursor-pointer">
                    <span className="text-lg">{resultIcons[r.type] || '📄'}</span>
                    <div>
                      <div className="text-sm font-medium text-gray-200">{r.name}</div>
                      {r.subtitle && <div className="text-xs text-gray-500">{r.subtitle}</div>}
                    </div>
                    <span className="ml-auto text-xs text-gray-600 capitalize">{r.type?.replace('_', ' ')}</span>
                  </Link>
                ))}
              </div>
            )}
            {searchQuery.length >= 2 && searchResults.length === 0 && (
              <div className="py-8 text-center text-gray-500 text-sm">No results for "{searchQuery}"</div>
            )}
            {searchQuery.length === 0 && (
              <div className="py-4 px-4">
                <div className="text-xs text-gray-600 mb-2">Quick links</div>
                <div className="grid grid-cols-2 gap-1">
                  {[['Dashboard', '/admin/dashboard'], ['Agents', '/admin/agents'], ['Activity', '/admin/activity'], ['Playground', '/admin/playground']].map(([name, path]) => (
                    <Link key={path} href={path} onClick={() => { setShowSearch(false); }} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-gray-800 text-gray-400 hover:text-gray-200 text-xs transition-colors">{name}</Link>
                  ))}
                </div>
              </div>
            )}
