        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-150 dark:border-gray-700 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
              <th className="py-4 px-6 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-850 select-none" onClick={() => handleSort('name')}>
                Name {sortField === 'name' && (sortOrder === 'asc' ? '▲' : '▼')}
              </th>
              <th className="py-4 px-6 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-850 select-none" onClick={() => handleSort('slug')}>
                Slug {sortField === 'slug' && (sortOrder === 'asc' ? '▲' : '▼')}
              </th>
              <th className="py-4 px-6 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-850 select-none" onClick={() => handleSort('plan')}>
                Plan {sortField === 'plan' && (sortOrder === 'asc' ? '▲' : '▼')}
              </th>
              <th className="py-4 px-6 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-850 select-none" onClick={() => handleSort('user_count')}>
                Users {sortField === 'user_count' && (sortOrder === 'asc' ? '▲' : '▼')}
              </th>
              <th className="py-4 px-6 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-850 select-none" onClick={() => handleSort('status')}>
                Status {sortField === 'status' && (sortOrder === 'asc' ? '▲' : '▼')}
              </th>
              <th className="py-4 px-6 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-850 select-none" onClick={() => handleSort('created_at')}>
          ...
          <tbody className="divide-y divide-gray-150 dark:divide-gray-750 text-sm">
            {paginatedTenants.length === 0 ? (
              ...
              paginatedTenants.map((t) => (
                <tr key={t.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-750/20 transition-all text-gray-700 dark:text-gray-300">
                  <td className="py-4 px-6 font-semibold text-gray-900 dark:text-white">{t.name}</td>
                  <td className="py-4 px-6 font-mono text-xs text-gray-500 dark:text-gray-400">{t.slug}</td>
                  ...
                  <td className="py-4 px-6 text-right space-x-2">
                    <button onClick={() => onView(t)} className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 font-semibold text-xs">
                      View
                    </button>
                    <button onClick={() => onEdit(t)} className="text-amber-600 dark:text-amber-400 hover:text-amber-900 dark:hover:text-amber-300 font-semibold text-xs">
                      Edit
                    </button>
                    {t.status !== 'suspended' && (
                      <button onClick={() => onSuspend(t)} className="text-rose-600 dark:text-rose-400 hover:text-rose-900 dark:hover:text-rose-300 font-semibold text-xs">
                        Suspend
                      </button>
                    )}
                  </td>
