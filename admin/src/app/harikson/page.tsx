...
export default function HariksonAdmin() {
  ...
  const [stats, setStats] = useState<
    (Stats & { pendingApprovals?: number; totalRevenue?: string }) | null
  >(null);
  ...
  >('STARTER');
  const [agentType, setAgentType] = useState<'CHAT' | 'CODING' | 'HYBRID'>(
    'CHAT'
  );
  ...
  const loadData = async () => {
    try {
      const [
        ...
        HariksonApiClient.get<
          Stats & { pendingApprovals?: number; totalRevenue?: string }
        >('/stats'),
      ...
      console.warn(
        'Harikson API offline, showing high-fidelity control panel simulator.'
      );
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await HariksonApiClient.post('/tenants', {
        name,
        plan,
        agentType,
        model,
  ...
  const handleSwitchBillingMode = async (
    mode: 'manual' | 'auto',
    gateway?: 'razorpay' | 'cashfree'
  ) => {
    try {
    ...
    <div>
      ...
      >
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: '700' }}>
            Neuravolt-Harikson AI Hub
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.9rem' }}>
            White-Label Multi-Tenant Orchestrator & Node Pools
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                {stats.allocations.cpuCores.toFixed(1)} Cores
              </h3>
            </div>
          </div>
        </div>
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Subdomain Prefix (lowercase, alphanumeric)
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                >
                  <option value="qwen3-coder-4b">
                    Harikson Pro (4B quantized)
                  </option>
                  <option value="qwen3-coder-8b">
                    Harikson Business (8B recommended)
                  </option>
                  <option value="qwen3-coder-14b">
                    Harikson Enterprise (14B)
                  </option>
                  <option value="qwen3-coder-32b">
                    Harikson Infinite (32B Enterprise only)
                  </option>
                </select>
              </div>
      ...
      {showEditForm && editingTenant && (
        ...
        >
          ...
          >
            ...
            >
              Updating plan resources or model variants will hot-reload the
              containers for <strong>{editingTenant.name}</strong>{' '}
              automatically.
            </p>
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <select
                    className="input-field"
                    value={editAgentType}
                    onChange={(e) => setEditAgentType(e.target.value as any)}
                  >
                    <option value="CHAT">CHAT AGENT</option>
                    <option value="CODING">CODING AGENT</option>
                    <option value="HYBRID">HYBRID MODE</option>
              ...
              >
                ...
                >
                  {actionLoading === editingTenant.id
                    ? 'Updating Stack...'
                    : 'Save Settings'}
                </button>
              </div>
      ...
      >
        ...
        <div className="glass-card" style={{ padding: '30px' }}>
          ...
          >
            Choose whether to require manual administrator verification or
            integrate automated credit/payment gateways for instant activations.
          </p>
          ...
          >
            ...
            >
              ...
              <div>
                ...
                >
                  Admin manually verifies client credentials and clicks approve
            ...
            <label
              style={{
                ...
                borderRadius: '10px',
                border:
                  billingMode.mode === 'auto' &&
                  billingMode.gateway === 'razorpay'
                    ? '1px solid #3b82f6'
                    : '1px solid rgba(255,255,255,0.06)',
                cursor: 'pointer',
                background:
            ...
            >
              ...
              <div>
                ...
                >
                  Enforces Razorpay checkout orders and updates deployment
                  statuses instantly on payment completion
                </span>
              </div>
            </label>
            ...
            >
              ...
              <div>
                ...
                >
                  Generates Cashfree API requests and releases nodes
                  automatically upon callback notifications
                </span>
              </div>
            </label>
          </div>
            ...
            <tbody>
              {tenants.length === 0 ? (
                <tr>
                  ...
                  >
                    No client agent stacks deployed. Click "Deploy AI Agent" to
                    start.
                  </td>
                </tr>
              ) : (
                tenants.map((t) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 10px' }}>
                      {t.status === 'RUNNING'
                        ? `${t.cpuUsage ?? 0}% / ${t.cpuLimit}`
                        : '-'}
                    </td>
                    <td style={{ padding: '14px 10px' }}>
                      {t.status === 'RUNNING'
                        ? `${t.memoryUsage ?? 0}MB / ${t.memoryLimit}`
                        : '-'}
                    </td>
                    <td style={{ padding: '14px 10px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        {t.status !== 'RUNNING' ? (
                          <button
                            onClick={() => handlePowerAction(t.id, 'start')}
                            className="btn btn-secondary"
                          ...
                          <button
                            onClick={() => handlePowerAction(t.id, 'stop')}
                            className="btn btn-secondary"
                            style={{ padding: '6px', borderRadius: '6px' }}
                            disabled={!!actionLoading}
                          >
                        ...
                        <button
                          onClick={() => handlePowerAction(t.id, 'restart')}
                          className="btn btn-secondary"
                          style={{ padding: '6px', borderRadius: '6px' }}
                          disabled={!!actionLoading}
                        >
                        ...
                        <button
                          onClick={() => handlePowerAction(t.id, 'delete')}
        ...
        >
          ...
          <div className="glass-card" style={{ padding: '24px 30px' }}>
            ...
            >
              ...
              <h3 style={{ fontSize: '1.1rem', fontWeight: '600' }}>
                Global Model Fine-Tuning Monitor
              </h3>
            </div>
                ...
                trainingJobs.map((j) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        className={`badge badge-${j.status === 'COMPLETED' ? 'running' : j.status === 'TRAINING' ? 'pending' : 'error'}`}
                        style={{ fontSize: '0.7rem' }}
                      >
                        {j.status}
                      </span>
                    ...
                    >
                      <div>
                        ...
                        <strong style={{ color: 'white' }}>
                          {getBrandedModelName(j.baseModel)}
                        </strong>
                      </div>
                      <div>
                        Adapter:{' '}
                    ...
                    {j.status === 'TRAINING' && j.metrics.loss && (
                      ...
                      >
                        Training Adapter progress: Epoch {j.metrics.epoch} |
                        Loss: {j.metrics.loss}
                      </div>
                    )}
                    {j.status === 'COMPLETED' && j.metrics.finalAccuracy && (
                      <div style={{ fontSize: '0.75rem', color: '#10b981' }}>
                        Adapters successfully compiled. Accuracy:{' '}
                        {(j.metrics.finalAccuracy * 100).toFixed(0)}%
                      </div>
                    )}
                  </div>
                ))
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        ...
                        <span>
                          {node.memory.usedGB.toFixed(1)}GB /{' '}
                          {node.memory.totalGB}GB
                        </span>
                      </div>
                      <div
                        style={{
                      ...
                      >
                        <div
                          style={{
                            width: `${(node.memory.usedGB / node.memory.totalGB) * 100}%`,
                            height: '100%',
                            background: '#3b82f6',
                          }}
                        />
